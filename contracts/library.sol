...
contract voltswap_library {
    voltswap_route internal router;

    constructor(address _router) {
        router = voltswap_route(_router);
    }
    ...
    function getTradeDiff(
        ...
        ) = voltswap_pair(router.pairFor(tokenIn, tokenOut, stable)).metadata();
        uint256 sample = tokenIn == t0
            ? (r0 * dec1) / r1 / 10000
            : (r1 * dec0) / r0 / 10000;
        a =
